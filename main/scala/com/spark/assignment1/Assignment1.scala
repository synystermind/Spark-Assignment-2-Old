...
object Assignment1 {
  ...
  /** Problem 2
    ...
    */
  def problem2(skaterStats: RDD[SkaterStats], game: RDD[Game]): Array[Int]  = {
    val mappedskaters = skaterStats.map(skater => (skater.game_id, skater))
    val mappedgames = game.map(game => (game.game_id, game))
    mappedskaters.join(mappedgames).count()
  }
  /** Problem 2 SQL
    * select distinct
    *   count(ss.player_id)
  ...
  /** Problem 3
    ...
    */
  def problem3(skaterStats: RDD[SkaterStats], playerInfo: RDD[PlayerInfo]): Long  = {
    ...
    mappedskaters.leftOuterJoin(mappedplayers).count()
  }
  /** Problem 3 SQL
    * select
    * pi.player_id
  ...
  /** Problem 4
    ...
    */
  def problem4(playerInfo: RDD[PlayerInfo]): Long = {
    playerInfo.filter(x => x.primaryPosition.equals("D")).count()
  }
  /** Problem 4 & 5 SQL
    * select
    * * pi.player_id
